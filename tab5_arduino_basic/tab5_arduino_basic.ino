...
Partition Scheme: "Custom" (the supplied partitions.csv file allows almost full use of the flash for
the main app) PSRAM: "Enabled" Upload Mode: "UART / Hardware CDC" USB Mode: "Hardware CDC and JTAG"

Notes:

This demo uses a software rotate in order to give us landscape mode (disp_drv.rotated =
LV_DISP_ROT_90;) I've not been able to find the make and model of the display unit used in the Tab5
so I can't be sure if it supports a hardware accelerated rotation. The display driver chip is
ili9881c with native portrait orientation.

A Square Line Studio V1.5.1 project is included so that you can experiment with your own exported UI
files.
    ...
    if (alarmEnabled) {
        ...
        if (alarmTriggered && !alarmSnoozed) {
            ...
            if (hours != alarmHours || minutes != alarmMinutes) {
                ...
                lv_obj_set_style_text_color(ui_AlarmLabel, lv_color_hex(0xFF6600),
        ...
        if (shouldTrigger) {
            ...
            lv_obj_set_style_text_color(ui_AlarmLabel, lv_color_hex(0xFF0000),
                                        LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    if (alarmTriggered) {
        ...
        if (currentTime - alarmStartTime >= alarmTimeout) {
            ...
            if (currentTime - lastFlashTime >= 500) {  // Flash every 500ms
                ...
                if (alarmFlashState) {
                    lv_obj_set_style_bg_color(ui_WakeUpPanel, lv_color_hex(0xFF0000),
                                              LV_PART_MAIN | LV_STATE_DEFAULT);
                } else {
                    lv_obj_set_style_bg_color(ui_WakeUpPanel, lv_color_hex(0xFFFF00),
                                              LV_PART_MAIN | LV_STATE_DEFAULT);
                }
            }
...
void screenClickHandler(lv_event_t *e) {
    ...
    if (timeSettingPanelVisible) {
        ...
        if (!lv_obj_has_flag(ui_TimeSettingPanel, LV_OBJ_FLAG_HIDDEN) &&
            !isChildOf(target, ui_TimeSettingPanel) && target != ui_TimeSettingPanel &&
            target != ui_SetTimeBtn && !isChildOf(target, ui_SetTimeBtn)) {
            lv_obj_add_flag(ui_TimeSettingPanel, LV_OBJ_FLAG_HIDDEN);
            timeSettingPanelVisible = false;
        }
    }
    ...
    if (alarmSettingPanelVisible) {
        ...
        if (!lv_obj_has_flag(ui_AlarmSettingPanel, LV_OBJ_FLAG_HIDDEN) &&
            !isChildOf(target, ui_AlarmSettingPanel) && target != ui_AlarmSettingPanel &&
            target != ui_SetAlarmBtn && !isChildOf(target, ui_SetAlarmBtn)) {
            lv_obj_add_flag(ui_AlarmSettingPanel, LV_OBJ_FLAG_HIDDEN);
            alarmSettingPanelVisible = false;
        }
    }
...
void usbFileSelected(lv_event_t *e) {
    ...
    if (btn == ui_USBFileList)
        return;  // Clicked on list background
...
void setup() {
    ...
    if (usbHost.begin()) {
        ...
        usbHost.setOnDriveConnected([](USBDriveInfo info) {
            ...
            Serial.print(info.capacity_mb);
            Serial.println(" MB");

            char info_str[256];
            snprintf(info_str, sizeof(info_str), "Drive: %s %s\nCapacity: %u MB / Free: %u MB",
                     info.vendor.c_str(), info.product.c_str(), info.capacity_mb,
                     info.free_space_mb);
