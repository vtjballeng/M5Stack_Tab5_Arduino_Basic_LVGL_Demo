 ...
 * Project-level configuration for M5Stack Tab5 Arduino LVGL Demo
 */

// Enable content (required for template-based config)
#if 1
 ...
 *If disabled, drawing on transparent background is more complex.*/
#define LV_COLOR_BLEND_MODE_LEGACY 0
 ...
 * - LV_LAYER_SIMPLE_FALLBACK_BUF_SIZE: [bytes]  used if `LV_LAYER_SIMPLE_BUF_SIZE` couldn't be
 * allocated.
 *
 ...
 *When LVGL calculates the gradient "maps" it can save them into a cache to avoid calculating them
 *again. LV_GRAD_CACHE_DEF_SIZE sets the size of this cache in bytes. If the cache is too small the
 *map will be allocated only while it's required for the drawing. 0 mean no caching.*/
#define LV_GRAD_CACHE_DEF_SIZE 0

/*Allow dithering the gradients (to achieve visual smooth color gradients on limited color depth
 *display) LV_DITHER_GRADIENT implies allocating one or two more lines of the object's rendering
 *surface The increase in memory consumption is (32 bits * object width) plus 24 bits * object width
 *if using error diffusion */
#define LV_DITHER_GRADIENT 0
#if LV_DITHER_GRADIENT
/*Add support for error diffusion dithering.
 *Error diffusion dithering gets a much better visual result, but implies more CPU consumption and
 *memory when drawing. The increase in memory consumption is (24 bits * object's width)*/
...
/*If you do not use the default thesaurus, be sure to use `lv_ime_pinyin` after setting the
 * thesaurus*/
